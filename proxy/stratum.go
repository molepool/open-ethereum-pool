...
func (s *ProxyServer) broadcastNewJobs() {
	...
	for m, _ := range s.sessions {
		...
		go func(cs *Session) {
			...
			if cs.PendingDiff {
				cs.diff = cs.nextDiff
				log.Printf("VarDiff change to %.8f for %s:%s", cs.diff, cs.login, cs.worker)
				cs.PendingDiff = false
			}

			reply := []string{t.Header, t.Seed, util.GetTargetHex(cs.diff), t.HeightHex}
